...
public class VersionDao extends baseDao {
	...
	public void add(Version version) throws SQLException {
		String sql="insert into version (pid,fid,createtime,userid,vtext,vparent,vcomment, vcount) values(?,?,?,?,?,?,?,0) ";
		...
		ps2.setDate(5, new Date(version.getCreatetime().getTime()));
	...
	public List<VersionLink> getVersionLinks(int pid, int fid) throws SQLException {
		...
		while(rs.next()) {
			...
			link.setCreatetime(rs.getTimestamp(4));
	...
	public VersionLink showVersion(int vid) throws SQLException{
		...
		if(rs.next())
		{
			...
			version.setCreatetime(rs.getTimestamp(4));
	...
	public int versionClone(List<Integer> vids) throws SQLException {
	    int count=0;
